...
| items[].summary | string | 日本語の要約（結論1文＋箇条書き3〜4項目・5行程度のプレーンテキスト） |
...
3. 「未要約」かつ「公開5日以内」の動画を集める（1チャンネル最大4本／回）。
4. 要約の優先順は「今回の新着（新しい順）→ 未要約の積み残し（新しい順）」。1回の実行で最大4件まで。
5. 各動画について Gemini API に **動画のYouTube URL** を渡し、日本語要約を生成する。
   - プロンプト:「動画を最後まで視聴し、日本語で短く。結論1文のあと要点を箇条書き3〜4項目
     （5行程度・重要な数字や固有名詞は残す・専門用語に補足）。述べられていないことは書かない。」
